...
class CORSScanner:
    def __init__(self, target_domain: str, max_concurrent: int = 50, live_urls_only: bool = False):
        self.target_domain = target_domain
        self.max_concurrent = max_concurrent
        self.live_urls_only = live_urls_only
        self.console = Console()
        self.vulnerabilities: List[CORSVulnerability] = []
        self.tested_urls: Set[str] = set()
        self.live_urls: Set[str] = set()
        
        # Sensitive endpoints that are likely to have valuable data
        self.sensitive_endpoints = [
            '/api', '/api/v1', '/api/v2', '/api/v3', '/rest', '/graphql',
            '/admin', '/dashboard', '/panel', '/control', '/manage',
            '/user', '/users', '/profile', '/account', '/settings', '/config',
            '/login', '/auth', '/oauth', '/token', '/session', '/jwt',
            '/upload', '/download', '/file', '/files', '/documents',
            '/data', '/database', '/db', '/backup', '/export', '/import',
            '/search', '/query', '/report', '/analytics', '/stats',
            '/payment', '/billing', '/invoice', '/order', '/transaction',
            '/customer', '/client', '/member', '/employee', '/staff',
            '/internal', '/private', '/secure', '/protected', '/restricted',
            '/debug', '/test', '/dev', '/staging', '/beta', '/preview',
            '/service', '/services', '/endpoint', '/webhook', '/callback',
            '/cart', '/checkout', '/purchase', '/subscribe', '/subscription',
            '/message', '/mail', '/email', '/notification', '/alert',
            '/log', '/logs', '/audit', '/monitor', '/health', '/status'
        ]
    ...
    async def crawl_urls(self, domains: Set[str]) -> Set[str]:
        ...
        # Use sensitive endpoints if live_urls_only flag is set
        endpoints_to_check = self.sensitive_endpoints if self.live_urls_only else [
    ...
    async def filter_sensitive_urls(self, urls: Set[str]) -> Set[str]:
        """Filter URLs to only include those with sensitive/valuable content"""
        if not self.live_urls_only:
            return urls
            
        sensitive_urls = set()
        semaphore = asyncio.Semaphore(self.max_concurrent)
        
        # Keywords that indicate sensitive content in responses
        sensitive_keywords = [
            'api', 'json', 'xml', 'token', 'auth', 'login', 'admin', 'dashboard',
            'user', 'profile', 'account', 'settings', 'config', 'database', 
            'password', 'email', 'phone', 'address', 'payment', 'credit',
            'session', 'cookie', 'jwt', 'oauth', 'key', 'secret', 'private',
            'internal', 'debug', 'test', 'dev', 'staging', 'beta', 'preview',
            'upload', 'download', 'file', 'document', 'backup', 'export',
            'query', 'search', 'data', 'information', 'report', 'analytics'
        ]
        
        async def check_url_sensitivity(session, url):
            async with semaphore:
                try:
                    async with session.get(url, timeout=aiohttp.ClientTimeout(total=15)) as response:
                        # Check status codes that indicate valuable content
                        if response.status in [200, 201, 202, 401, 403]:
                            # Get response headers and content
                            headers = dict(response.headers)
                            content_type = headers.get('content-type', '').lower()
                            
                            # Check for API endpoints or JSON responses
                            if any(api_type in content_type for api_type in ['json', 'xml', 'api']):
                                sensitive_urls.add(url)
                                return
                            
                            # Check for authentication-related headers
                            auth_headers = ['www-authenticate', 'authorization', 'x-api-key', 'x-auth-token']
                            if any(header in headers for header in auth_headers):
                                sensitive_urls.add(url)
                                return
                            
                            # Check for CORS headers (indicates API endpoint)
                            ...
                            # For 200 responses, check content for sensitive keywords
                            if response.status == 200:
                                try:
                                    content = await response.text()
                                    ...
                                    # Check for structured data patterns (JSON-like)
                                    ...
                                    # If we can't read content but got 200, it might still be valuable
                                    if response.status == 200:
                                        sensitive_urls.add(url)
                            
                            # 401/403 often indicate protected resources
        ...
        async with aiohttp.ClientSession() as session:
            with Progress(
                SpinnerColumn(),
                TextColumn("[bold magenta]🎯 Filtering for sensitive endpoints..."),
                BarColumn(),
                TextColumn("[progress.percentage]{task.percentage:>3.0f}%"),
                TimeRemainingColumn(),
            ) as progress:
                ...
                tasks = []
                for url in urls:
                    tasks.append(check_url_sensitivity(session, url))
...
╚═════════════════════════════════════════════════════════════════════╝
        ...
        self.console.print(f"[bold]🎯 Sensitive URLs Only:[/bold] {'Yes' if self.live_urls_only else 'No'}")
        self.console.print()
        ...
        try:
            ...
            self.console.print(f"✅ Discovered {len(all_urls)} URLs")
            
            # Step 3.5: Filter for sensitive URLs if live_urls_only flag is set
            if self.live_urls_only:
                self.console.print("\n[bold magenta]Phase 3.5: Sensitive Content Filtering[/bold magenta]")
                all_urls = await self.filter_sensitive_urls(all_urls)
                self.console.print(f"🎯 Filtered to {len(all_urls)} sensitive URLs")
            else:
                self.console.print(f"✅ Total URLs to test: {len(all_urls)}")
...
Examples:
  ...
  python corsover.py -d example.com -L (sensitive URLs only - recommended for bug bounty)
  python corsover.py -d example.com -L -c 75
        """
    )
    ...
    parser.add_argument('-L', '--live-urls-only', action='store_true',
                       help='Only test URLs with sensitive/valuable content (recommended for bug bounty)')
    ...
    scanner = CORSScanner(domain, args.max_concurrent, args.live_urls_only)
